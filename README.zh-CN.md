...
- **可扩展模板**：借助 `repoctl` 提供的脚本（`script:init`、`script:sync`、`script:clean` 等）维护依赖与脚手架。`repo` 可作为短别名，`rc` 和 `@icebreakers/monorepo` 保留为兼容入口。
...
4. **创建下一个包或应用**：运行 `pnpm exec repoctl new` 进入引导式创建流程。
5. **本地开发**：使用 `pnpm dev` 启动 Turbo 并行开发任务，在各应用内快速迭代。
6. **构建与验证**：依次运行 `pnpm exec repoctl check`、`pnpm build`、`pnpm test`、`pnpm lint`。
7. **模板清理（可选）**：执行 `pnpm script:clean` 清理示例包，为自定义项目腾出空间。
...
- 零安装清理：`pnpm dlx repoctl@latest clean --yes`，需要保留 private 包时追加 `--include-private`。
  短别名也可使用：`pnpm dlx repo@latest clean --yes`。`rc` 不作为主推荐入口，主要是为了降低和其他全局命令冲突的概率。
...
| `pnpm exec repoctl init`      | 初始化推荐的 workspace 元数据与 tooling。 |
| `pnpm exec repoctl new`       | 使用引导式流程创建新的包或应用。          |
| `pnpm exec repoctl check`     | 执行推荐的本地校验。                      |
| `pnpm script:init`            | 通过兼容脚本初始化模板配置。              |
| `pnpm script:sync`            | 通过 `repoctl` 同步仓库资产与脚本。       |
...
- 在已有工作区中，优先使用 `pnpm exec repoctl init`、`pnpm exec repoctl new` 与 `pnpm exec repoctl check` 作为默认维护路径。
...
- **持续升级**：执行 `npx repoctl@latest` 获取模板最新能力；如果偏好更短命令，也可使用 `npx repo@latest`。
