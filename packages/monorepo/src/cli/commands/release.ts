...
export function registerReleaseCommands(program: Command, cwd: string) {
  ...
  const stableCommand = releaseCommand.command('stable')
    ...
    .action(async () => {
      await runReleaseAction(async () => {
        const { publishStable, runAfterPublishHooks } = await import('@/commands')
        const releaseOptions = await resolveReleaseOptions(cwd)
        runAfterPublishHooks(await publishStable(releaseOptions), releaseOptions)
  ...
  stableCommand.command('prepare')
    ...
    .action(async () => {
      await runReleaseAction(async () => {
        ...
        await prepareStable(await resolveReleaseOptions(cwd))
  ...
  preCommand.command('publish')
    ...
    .action(async () => {
      await runReleaseAction(async () => {
        const { releasePrerelease, runAfterPublishHooks } = await import('@/commands')
        const releaseOptions = await resolveReleaseOptions(cwd)
        runAfterPublishHooks(await releasePrerelease(releaseOptions) ?? [], releaseOptions)
