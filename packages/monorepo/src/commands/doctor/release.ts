...
export async function collectReleaseChecks(workspaceDir: string, pkgJson: PackageJsonLike) {
  ...
  if (workflowStatus === 'managed') {
    checks.push(check('release-workflow', 'release workflow', 'pass', `release workflow 已由 ${releaseWorkflowMarker} 管理。`))
  }
    ...
    checks.push(check(
      ...
      'release workflow',
      'warn',
      '检测到旧的 Changesets release workflow。',
      '首次迁移运行 pnpm dlx repoctl@latest upgrade --yes。',
    ))
  }
    ...
    checks.push(check(
      ...
      'release workflow',
      'warn',
      'release workflow 未包含 repoctl managed marker，可能存在自定义发布逻辑。',
      '确认内容后运行 repo upgrade --overwrite-release，或手动加入 repoctl-managed: release/v2。',
    ))
  }
  ...
  if (hasPreState) {
    checks.push(check(
      ...
      'prerelease state',
      'warn',
      '检测到旧的 .changeset/pre.json。',
      '运行 pnpm dlx repoctl@latest upgrade --yes 将其迁移为 pnpm versioning.lanes。',
    ))
  }
  if (hasChangesetConfig) {
    checks.push(check(
      ...
      'changeset config',
      'warn',
      '检测到只属于 Changesets CLI 的 .changeset/config.json。',
      '运行 pnpm dlx repoctl@latest upgrade --yes 清理旧配置。',
    ))
  }

  if (hasWorkflow) {
    const packageVersion = pkgJson.devDependencies?.['repoctl'] ?? pkgJson.dependencies?.['repoctl']
    const parsed = minVersion(packageVersion ?? '')
    if (!parsed || !gte(parsed, releaseCiMinimumVersion)) {
      checks.push(check(
        'release-cli-version',
        'release CLI version',
        'warn',
        `repoctl 版本 ${packageVersion ?? 'missing'} 不保证支持 release ci。`,
        `运行 pnpm dlx repoctl@latest upgrade --yes，或将 repoctl 升级到 >=${releaseCiMinimumVersion}。`,
      ))
    }
    else {
      checks.push(check('release-cli-version', 'release CLI version', 'pass', `repoctl 版本 ${packageVersion} 支持 release ci。`))
    }
    ...
    try {
      ...
      checks.push(
        hasFixed && hasRepositoryChangelog
          ? check('release-versioning-config', 'release versioning config', 'pass', 'pnpm fixed group 与 repository changelog 配置完整。')
          : check('release-versioning-config', 'release versioning config', 'warn', 'pnpm-workspace.yaml 缺少 release 所需的 fixed 或 repository changelog 配置。', '运行 repo upgrade --yes 同步 pnpm versioning 配置。'),
      )
    }
    catch {
      checks.push(check('release-versioning-config', 'release versioning config', 'warn', '无法读取 pnpm-workspace.yaml 的 versioning 配置。', '运行 repo upgrade --yes 同步 pnpm versioning 配置。'))
    }
  }
