...
pnpm exec repo env paths --markdown --redact --out reports/paths.md
pnpm exec repo config inspect --json --out reports/config.json
pnpm exec repo config inspect --markdown --redact --out reports/config.md
...
如果要把环境报告发给外部协作者，优先加 `--redact` 脱敏本机绝对路径；如果是贴到 issue 或 PR 评论里，用 `--markdown` 生成摘要。
在 CI 中可以再加 `--strict`，这样会先写出排障包，再在 `doctor` 有 fail 或 warn 时返回失败码。
